...
// What is the avg age of the users ?
...
db.getCollection("users").aggregate([
    {
        $addFields: {
          numberOfTags:{
            $size:{
             // you have to handle a case that if tags is not present in the document if it is null treate it as any empty array
             $ifNull:["$tags",[]]   
            }
          }
        }
...
// How mant users have 'enim' as there tags
db.getCollection("users").aggregate([
    {
        $match:{
            tags:"enim"
    ...
    {   // this is count field name userWithTagAsEnim
        $count:"userWithTagAsEnim"
    }
])


// What are the names and age of the users who are inactive and has velit as there tag ? 
...
db.getCollection("users").aggregate([
    ...
    {
        $project: {
            // What are the field that i want i have to make it 1 it could be field that exists in the doc or a newly created one
          name:1,
          age:1,
          _id:0
        }
...
// How many users have there phone number starts with +1(940) ?
db.getCollection("users").aggregate([
    {
        $match:{
            "company.phone":/^\+1 \(940\)/
    ...
    {
        $count:"userWithPhoneStartingWith940"
    }
])
...
// Categorize user by there fav fruits ?
db.getCollection("users").aggregate([
    {
        $group: {
          _id: "$favoriteFruit",
...
// How many users have ad as there second tag
...
// Find the users who have both enim and id as there tags
db.getCollection("users").aggregate([
    {
        $match:{
            $and:[{tags:"enim"},{tags:"id"}] // this is the one way to do it
            // tags:{
            //     $all:["enim","id"]
            // }
        }
...
// How to use lookup in aggregation.
